...
package object arithmetic {
  ...
  implicit class S99Int(val value: Int) extends AnyVal {
    ...
    /** =P37 (**) Calculate Euler's totient function phi(m) (improved)=
      ...
      * efficiently calculated as follows: Let {{{[[p1, m1], [p2, m2], [p3, m3], ...]}}} be the
      * list of prime factors (and their multiplicities) of a given number ''m''. Then `phi(m)` can
      * be calculated
      ...
      */
    def phi: Int = (primeFactorMultiplicity fold 1) { (prod, e) =>
      val (p, m) = e
      prod * (p - 1) * math.pow(p, m - 1).toInt
    }
    ...
    def goldbach: (Int, Int) = {
      ...
      def loop(a: Int): (Int, Int) =
        if (a.isPrime && (value - a).isPrime) (a, value - a)
        else loop(a + 1)
  ...
  /** =P39 (*) A list of prime numbers=
    ...
    * }}}
    * @note filter where created
    * @todo create Range type
    */
  def listPrimesInRange(from: Int, to: Int): List[Int] = List.range(from, to) filter (_.isPrime)
  ...
  /** =P41 (**) A list of Goldbach compositions=
    ...
    * 1928 = 61 + 1867
    * }}}
    * @todo implement withFilter, foreach
    * @todo generic print
    */
  def goldbachList(from: Int, to: Int): List[(Int, (Int, Int))] =
    List.range(from, to) filter (_ % 2 == 0) map (e => (e, e.goldbach))

  def goldbachListLimited(from: Int, to: Int, limit: Int): List[(Int, (Int, Int))] =
    goldbachList(from, to) filter (_._2._1 >= limit)

  def printGoldbachList(from: Int, to: Int): Unit =
    goldbachList(from, to) map { case (x, (a1, a2)) => println(s"$x = $a1 + $a2") }

  def printGoldbachListLimited(from: Int, to: Int, limit: Int): Unit =
    goldbachListLimited(from, to, limit) map { case (x, (a1, a2)) => println(s"$x = $a1 + $a2") }
