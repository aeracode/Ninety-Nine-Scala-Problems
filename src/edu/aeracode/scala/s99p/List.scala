...
import edu.aeracode.scala.s99p.List.notDefined
...
sealed trait List[+E] {
  ...
  def :++[A >: E](l: L[A]): L[A]
  def :-[A >: E](e: A): List[A]
  def :--[A >: E](es: L[A]): L[A] = (es fold[L[A]] this) {_ :- _}
  ...
  /** =P12 (**) Decode a run-length encoded list=
    *
    * Given a run-length code list generated as specified in problem P10, construct its uncompressed
    * version.
    *
    * @example
    * {{{
...
object List {
  ...
  private[s99p] def notDefined(s: String): Nothing =
